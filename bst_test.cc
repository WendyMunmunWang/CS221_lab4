...
bool delete_node(Node*& root, KType key) {
  ...
  // case 2: target has two children
    ...
    KType Newkey = ptr->key;
    delete_node(root , Newkey);
    target->key = Newkey;
...
int numNodes( Node* root ) {
  ...
  count_Nodes++;
  if(count_N->left){
    count_Nodes += numNodes(count_N->left);
  }
  if(count_N->right){
    count_Nodes += numNodes(count_N->right);
  }
  return count_Nodes;
}
...
int numLeaves( Node* root ) {
  ...
  int count_leaves = 0;  
  if(!count_l) return 0;
  if(!count_l->left && !count_l->right) count_leaves++;
  if(count_l->left) count_leaves += numLeaves(count_l->left);
  if(count_l->right) count_leaves += numLeaves(count_l->right);
  return count_leaves;
}
...
int height( Node* x ) {
  ...
  if(height_l>height_r) return height_l;
  else return height_r;
}
...
int depth( Node* x , Node* root ) {
  ...
  if(!root) return 0;
  int depth_n = 0;
  if(root == x) return depth_n;
  if(x == root->left || x == root->right){
    depth_n++;
    return depth_n;
  }
  depth_n += depth(x,root->left);
  depth_n += depth(x,root->right);
  if(depth_n != 0) depth_n++;
  return depth_n;
}
