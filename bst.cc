...
int numNodes( Node* root ) {
	 ...
	 */
	 ...
	 Node *ptr = root;   // a current pointer representing where you are
	 int counts=0;
	 if (root==NULL) return 0;
	 if(ptr->left!=NULL) {counts = counts+numNodes(ptr->left);}
	 if(ptr->right!=NULL) {counts = counts+numNodes(ptr->right);}
	 return 1+counts;
	 }
...
int numLeaves( Node* root ) {
	 ...
	 */
	 ...
	 int counts = 0;
	 if (root==NULL) return 0;
	 if (ptr->left==NULL&&ptr->right==NULL) counts=counts+1;
	 if (ptr->left!=NULL) counts = counts+numLeaves(root->left);
	 if (ptr->right!=NULL) counts = counts+numLeaves(root->right);
	 return counts;
}
 ...
 */
 //The height of any tree is defined as the greatest path-length from root-to-leaf
 //A tree with only one node has height = 0 and an empty tree has height = -1.
int height( Node* x ) {
	/**
	 * THIS FUNCTION NEEDS TO BE IMPLEMENTED ********************
	 */
	 ...
	 if (x->left!=NULL) {
		 height_counts = 1+height_counts + height(x->left);
	 }
	 else height_counts = 1+height_counts+height(x->right);
...
int depth( Node* x , Node* root ) {
	 ...
	 */
	int depth_counts = 0;
	if (x==NULL||root==NULL) return -1;  //empty tree
	if (root==x) return 0;
    if (x->key<root->key) depth_counts=1+depth_counts+depth(x, root->left);
    else depth_counts=1+depth_counts+depth(x,root->right);
	return depth_counts;
}
...
void in_order( Node*& rootNode, int level, Visitor& v ) {
	 ...
	 */
	 if (rootNode!=NULL) {
		 ...
		 in_order(rootNode->right, height(level),v);
...
void pre_order( Node*& rootNode, int level, Visitor& v ) {
	 ...
	 * THIS FUNCTION NEEDS TO BE IMPLEMENTED ********************
	 */
	 // Pre-order traversal happens by first “visiting” the node
     // itself, then traversing the left subtree, and finally traversing the right subtree.
	 if (rootNode!=NULL){
		 v.visit(rootNode, level);
		 in_order(rootNode->left, level, v);
